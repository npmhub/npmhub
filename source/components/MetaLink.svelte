<script>
  ...
  // Local variables used to change style classes for elements
  export let divClass;
  export let linkClass;
  export let svgClass;

  if (isPackageJson || isTree) {
    divClass  = "ml-3";
    linkClass = "Link--primary";
    svgClass  = "octicon";
  } else {
    divClass  = "mt-3";
    linkClass = "Link--muted";
    svgClass  = "octicon mr-2";
  }
</script>

<h3 class="sr-only">Dependency List via N P M Hub</h3>
<div class={divClass}>
  <a href="#dependencies" class={linkClass}>
    <svg version="1.1" aria-hidden="true" height="16" width="16" viewBox="150 50 250 450" data-view-component="true" class={svgClass}>
